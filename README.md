...
By doing so they also gain one credit token (ERC20) that can be used to revive a chosen cell
...
Anyone can progress a game, and in return they get credit tokens. Ten
...
Contract has following external functions:

1. Initializer - ***can only be called once, right after contract deploy, by the contract owner/admin***

Initializes the contract and creates the infinite game.
...
```
func upgrade(
        implementation_hash (felt): Hash of new contract implementation
    ):
```

3. Evolve

Evolves the game and rewards user with credit token.
...
Example:
```
$ # Evolve infinite game
$ starknet --account user1 invoke --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function evolve --inputs 39132555273291485155644251043342963441664
```

4. Create - ***only creator mode***

Creates a new game, requires user to have at least 10 credit tokens.
...
Example:
```
$ # Create a new game with single cell alive at index 16 - first column, second row
$ # Such game is encoded in value 32768
$ starknet --account user1 invoke --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function create --inputs 32768
```

5. Give life to cell - ***only infinite mode***

Gives life to cell under chosen index, requires user to have at least 1 credit token.
```
...
$ starknet --account user1 invoke --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function give_life_to_cell --inputs 100
```
...
Example:
```
$ # Show inifinite game at first generation. The game state should be the same as game_id.
$ starknet --account user1 call --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function view_game --inputs 39132555273291485155644251043342963441664 1
...
$ starknet --account user1 call --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function view_game --inputs 39132555273291485155644251043342963441664 2
...
$ starknet --account user1 call --address 0x06dc4bd1212e67fd05b456a34b24a060c45aad08ab95843c42af31f86c7bd093 --abi build/gol_abi.json --function get_current_generation --inputs 39132555273291485155644251043342963441664
...
- Users can earn credit tokens by evolving games, and spend them by creating new games/reviving cells in infinite mode
...
- `INFINITE_GAME_GENESIS` - The genesis state for the game in infinite mode. This state is also used as game_id for infinite game.
- `DIM` - The game grid dimensions
- `CREATE_CREDIT_REQUIREMENT` - Credit token count required for new game creation
- `GIVE_LIFE_CREDIT_REQUIREMENT` - Credit token count required for giving life in infinite mode
- `FIRST_ROW_INDEX` - Index of the first row of the game grid; Is 0 and should stay 0 even when `DIM` change
- `LAST_ROW_INDEX` - Index of the last row of the game grid; Should be `DIM - 1`
- `LAST_ROW_CELL_INDEX` - Index of the first cell in the last row of the game grid; Should be `DIM - DIM * DIM`
- `FIRST_COL_INDEX` - Index of the first column of the game grid; Is 0 and should stay 0 even when `DIM` change
- `LAST_COL_INDEX` - Index of the last column of thr game grid; Should be `DIM - 1`
- `LAST_COL_CELL_INDEX` - Index of the last cell if the last column of the game grid; Should be `DIM - 1`
- `SHIFT` - Shift number used to pack game into one felt; Has to be `2**128`
- `LOW_ARRAY_LEN` - Length of an array that represents the "low" value of the game board; Max value is `128`, added to `HIGH_ARRAY_LEN` has to be equal to `DIM**2`
- `HIGH_ARRAY_LEN` - Length of an array that represents the "high" value of the game board; Max value is `128`, added to `LOW_ARRAY_LEN` has to be equal to `DIM**2`

#### Events that are emitted by the contract. These are later parsed by the indexer. (defined in `utils/events.cairo`). <a name="events"></a>

- `game_created` - Indicates new creator game was created
    - values:
...
To pack the game into one felt we use `pack_game` function.

It takes an array with cells (represented as binary values, where 0 is dead cell and 1 is alive cell) and splits it into two, smaller arrays of lengths `LOW_ARRAY_LEN` and `HIGH_ARRAY_LEN`. So, for example, from input game array (acorn):
...
Each array is then passed to `pack_cells` function, that converts the binary representation of the array into its decimal representation. In order to calculate it, we need to **reverse** the order of cells inside the arrays first, so the low array representation becomes:
```
...
```
So, in decimal representation, `83077383561857356171188689619124224` and `115` respectively.
...
```
Substituting with values calculated above we get `39132555273291485155644251043342963441664` which is our game board packed into single felt.


#### Unpacking game <a name="unpacking_game"></a>

To unpack the game from single felt to an array with cell states we use `unpack_game` function.

It takes a felt with packed game and splits it into two values describing "high" array part of the board and "low" array part of the board (this is done using built in function [`split_felt`](https://github.com/starkware-libs/cairo-lang/blob/167b28bcd940fd25ea3816204fa882a0b0a49603/src/starkware/cairo/common/math.cairo#L122)).
